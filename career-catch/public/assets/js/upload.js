...
//지도 option 설정
const mapOption = {
  center: new daum.maps.LatLng(37.4510277552463,127.1295999693142), //맵의 Defualt를 가천대학으로
  level: 3, //지도의 초기 줌 레벨
};
...
//검색 키워드로 장소를 검색하는 searchPlaces() 함수 호출
searchPlaces();

//검색 키워드로 장소를 검색하는 searchPlaces()함수 선언
function searchPlaces() {
  //upload.hbs 키워드 검색창(#keyword)에서 검색 키워드(value)를 가져옴
...
function displayPlaces(data) {
   //결과를 출력할 DOM 요소(placesList) 참조 - upload.hbs
...
$(".fixed2").click(function(){
  $(".fixed2").addClass("unvisible");
  $(".box").removeClass("unvisible");
});

$(".click").click(function(){
  $(".fixed2").removeClass("unvisible");
  $(".box").addClass("unvisible");
});
