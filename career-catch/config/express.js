...
// ===== flash 메시지(일회성 메시지)를 보내기 위한 'connect-flash' 모듈 참조 ====//
...
module.exports = (app, passport) => {
 ...
 /* express-session을 미들웨어에 등록(옵션 설정)  
     - session ID(connect-sid)는 cookie에 저장되고, session DATA는 서버에 저장
     - 실무에서는 일반적으로 메모리DB(etc. redis) 사용하지만 실습에선 mongodb를 이용
 */
  //============================================================
   app.use(
      session({
      secret: ENV.SESSION_SECRET, //쿠키에 저장할 connect-sid 암호화 key값(secret key)
      resave: false, //매번 request마다 세션에 변화가 없어도 다시 저장하는 옵션
      saveUninitialized: false, //uninitialized 상태(세션에 아무런 작업이 이루어 지지 않은 상황)의 세션을 강제 저장하는 옵션
      store: MongoStore.create({
         //세션 데이터 저장소 지정( MongoDB에 저장하기 때문에 mongo 접속정보)
